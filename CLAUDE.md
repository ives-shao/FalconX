...
3.1 读取范围遵循 [docs/process/AI协作与提示规范.md](docs/process/AI协作与提示规范.md) 中的“按任务类型最小阅读集”；允许按任务缩小阅读集，但不得跳过与当前改动直接相关的正式规范。
...
11.1 对不改变接口契约、数据库 schema、Kafka payload、错误码、状态码、包结构与 owner 边界的内部实现、同 owner 受控重构、测试/日志补齐、文档澄清和提示词优化，代理可直接实施，不需要因算法、私有方法拆分、注释风格、验证命令选择等内部细节重复向用户确认。
...
20.1 验证强度必须与改动范围相称：纯文档 / 提示规范任务以文档一致性核对为主；单服务改动优先做模块级验证；只有跨服务、缓存、事件、schema 或基础设施变更才升级到联通性验证。
