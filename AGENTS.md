...
✅ **允许快速通道：**
- 所有 [§ 3.1 文档读取规则](#31-文档读取规则) 中定义的"🟢 可选读取"场景
- 同一会话中已经读取过相关文档的后续修改

❌ **不允许快速通道：**
- 所有 [§ 3.1 文档读取规则](#31-文档读取规则) 中定义的"🔴 强制读取"场景
- 跨服务边界的修改

> 💡 **详细规则**：查看 [§ 3.1 文档读取规则](#31-文档读取规则) 或 [规则速查表](docs/process/规则速查表.md)
...
3.3.5 允许配置文件定义测试产品清单（必须明确标注为测试数据）。

#### 🟢 开发环境可选

3.3.6 允许使用 `List.of(...)` 快速验证功能。

3.3.7 允许硬编码少量测试数据，但必须在代码中明确标注：`// TODO: Replace with owner data source before staging`

3.3.8 必须在进入staging前移除所有硬编码测试数据。
...
9.3 若使用 Git 平台提交流程，优先使用仓库中的 PR 模板。

9.4 不允许以"后续再补文档/测试/日志"为理由跳过当前交付要求。
