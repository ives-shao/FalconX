...
- 当前处于 `Stage 6A` 部分完成状态，已从 stub/骨架进入 EVM 原生币最小真实链路。
- 运行时已具备真实节点连接能力，并通过 `t_wallet_chain_cursor` 驱动扫块游标。
- 当前实现聚焦“EVM 原生币原始事实”，尚未扩展到完整代币解析和多链全量能力。

## 已落地能力

- 钱包地址分配与 owner MySQL 持久化已落地。
- 已支持 EVM `http / https / ws / wss` 连接方式。
- 已具备按游标扫块、预载平台地址快照、识别平台地址原生币入金的最小主链路。
...
- ERC20 日志解析与代币精度处理尚未完成。
- 更高阶确认推进和更完整的链重组处理尚未完成。
