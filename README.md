...
- `Stage 6A`：Tiingo 外部真源自动化验证、外部链节点真扫块自动化验证、以及 `Swap / 隔夜利息` 的规则确认与实现仍未完成；但当前阶段主链路已按正式口径收口为“除 Swap 外主链路收口完成”。
