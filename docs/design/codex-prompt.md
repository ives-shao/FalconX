# FalconX 前端产品设计方案 —— Codex 执行提示词（v1）

> 这份文件是投喂给本机 Codex 的主 prompt。Codex 读完本文件就应具备：
> - 清晰的角色定位
> - 完整的业务上下文与后端契约边界
> - 可直接落地的交付清单与目录结构
> - 明确的验收标准与红线
>
> **Codex 请严格按本 prompt 的 §0 → §12 顺序执行。**
> 任何与本文件冲突的默认习惯一律让位于本文件；本文件与 `FalconX/CLAUDE.md` 冲突时，以 `CLAUDE.md` 为准。
...
以下文档是 FalconX 仓库的**正式契约来源**，不是参考资料。开始设计前请**逐份读完**，读完后用一段不少于 300 字的总结说明"你掌握到的关键约束"，再进入 §3。
...
- `docs/api/FalconX统一接口文档.md`（**最重要**：所有已实现接口的真实契约）
